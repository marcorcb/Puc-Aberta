...
class BaseViewController: PAViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let navigationController = self.navigationController, navigationController.viewControllers.count > 1 {
            self.navigationItem.leftBarButtonItem = UIBarButtonItem.barButton(withImage: #imageLiteral(resourceName: "icon_back"), target: self,
                                                                              action: #selector(goBack))

            weak var weakNav = self.navigationController
            self.navigationController?.interactivePopGestureRecognizer?.delegate = weakNav
