...
public class APIRequest {
    ...
    struct Constants {
        static let baseURL = URL(string: "http://portal.pucminas.br/pucaberta/")!
    ...
    var cacheFileName: String {
        return APICacheManager.shared.cacheFileNameWith(path: self.path, method: self.method.rawValue,
                                                        parameters: self.parameters)
    }

    init(method: API.HTTPMethod, path: String, parameters: [String: Any]?, urlParameters: [String: Any]?,
         cacheOption: API.CacheOption, completion: ResponseBlock<Any>?) {
    ...
    func showErrorMessage(error: API.RequestError) {
        ...
        DispatchQueue.main.async {
            let alertController = PAAlertController(titleImage: #imageLiteral(resourceName: "icon_error"), message: errorMessage)
    ...
    func handleSuccess(data: Data?, response: URLResponse) {
        ...
        for headerName in Constants.authenticationHeaders {
            if let headerValue = response.allHeaderFields[headerName] {
                ...
                guard var storedAuthenticationHeaders = UserDefaults.standard.dictionary(forKey:
                    Constants.authenticationHeadersDefaultsKey) else { break }
                storedAuthenticationHeaders[headerName] = headerValue
                UserDefaults.standard.set(storedAuthenticationHeaders,
                                          forKey: Constants.authenticationHeadersDefaultsKey)
            }
        }
    ...
    func handleError(data: Data?, response: URLResponse?, error: Error?) {
        ...
        let error = API.RequestError(responseObject: responseObject, urlResponse: response as? HTTPURLResponse,
                                     originalError: error)
    ...
    func makeRequest() {
        ...
        if self.cacheOption == .both || self.cacheOption == .cacheOnly {
            hasCache = APICacheManager.shared.callBlock(self.completionBlock,
                                                        ifCacheExistsForFileName: self.cacheFileName)
        }
        ...
        if self.cacheOption == .both || self.cacheOption == .networkOnly || !hasCache {
            ...
            for headerName in Constants.authenticationHeaders {
                let headerValue = UserDefaults.standard.dictionary(forKey:
                    Constants.authenticationHeadersDefaultsKey)?[headerName]
...
extension API.RequestError: LocalizedError {
    ...
    public var errorDescription: String? {
        if let responseObject = self.responseObject as? [String: Any], let error = responseObject["error"] as? String {
            return error
        } else if let responseObject = self.responseObject as? [String: Any],
            let errors = responseObject["errors"] as? [String] {
            return errors[0]
